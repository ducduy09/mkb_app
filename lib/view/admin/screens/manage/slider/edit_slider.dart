...
class EditSlider extends StatefulWidget {
  ...
  const EditSlider({Key? key, required this.data, required this.id}) : super(key: key);
...
class _EditSliderState extends State<EditSlider> {
  ...
  @override
  void initState() {
    linkController.text = widget.data[widget.id]["image_path"];
    super.initState();
    fToast = FToast();
    fToast.init(context);
  }
  Widget _submitButton() {
    return InkWell(
      onTap: () {
          ...
          // );
          fToast.showToast(
            child: const Text('Add product successfully !!', style: TextStyle( color: Colors.white,)),
